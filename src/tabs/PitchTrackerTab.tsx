  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 style={{ color: '#003E52' }}>Pitch Tracker</h2>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <input
          ...
          style={{ borderColor: '#E5E7EB', color: '#003E52', minWidth: '220px' }}
        />
        <select
          value={statusFilter}
        ...
        <span className="text-sm self-center" style={{ color: '#6B7280' }}>
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-12 rounded-xl animate-pulse" style={{ backgroundColor: '#E5E7EB' }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
          ...
          <table className="dash-table w-full">
            ...
            <tbody>
              {filtered.map((pitch, idx) => (
                <tr key={idx}>
                  <td className="font-medium" style={{ color: '#003E52' }}>
                    {`${pitch.journalistFirst} ${pitch.journalistLast}`.trim() || <span style={{ color: '#6B7280' }}>—</span>}
                  </td>
                  <td>{pitch.outlet || <span style={{ color: '#6B7280' }}>—</span>}</td>
                  <td>
                    <span
                      className="text-xs px-2 py-0.5 rounded font-medium"
                      style={{ backgroundColor: 'rgba(200,164,90,0.1)', color: '#003E52' }}
                    >
                      {pitch.omniProperty}
                    </span>
                  </td>
                  <td className="max-w-xs" style={{ maxWidth: '260px' }}>
                    <span
                      ...
                      style={{ color: '#003E52' }}
                    >
                      {pitch.subjectLine || <span style={{ color: '#6B7280' }}>(no subject)</span>}
                    </span>
                  </td>
